...
async function main(){
 ...
 {check:'Differences are highlighted in the shared answer',ok:qa.some(f=>f.judges?.highlightInSharedAnswer&&f.text.some(x=>x.text==='Finding differences'&&x.alpha>.9)&&f.text.some(x=>x.text==='Contradiction'&&x.alpha>.9))},
 {check:'Detected differences have no redundant closing tagline',ok:qa.every(f=>!f.text.some(x=>x.text==='Same goal. Different timing.'))},
