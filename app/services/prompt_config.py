...
def validate_config(data):
    if not isinstance(data, dict) or set(data) != {"reference_timezone", "prompts"}:
    ...
    for key, text in prompts.items():
        ...
        result[key] = text
    return {"reference_timezone": name, "prompts": result}


def _snapshot(document):
    if not document:
        return {**defaults(), "revision": 0, "updated_at": None, "updated_by": None}
    config = validate_config({key: document.get(key) for key in ("reference_timezone", "prompts")})
...
class PromptConfigStore:
    ...
    def save(self, config, *, expected_revision, updated_by):
        ...
        def operation(tx):
            ...
            if all(current[key] == config[key] for key in config):
                # An initial explicit Save should still create the database record.
                if current["revision"]:
                    return current
            document = {**config, "revision": current["revision"] + 1,
